...
Кто и что может по каждому из 52 эндпоинтов — [`docs/ACCESS-MAP.md`](docs/ACCESS-MAP.md). Файл собирается из кода, руками не поддерживается:

```bash
npm run audit:routes        # пересобрать карту доступа
npm run audit:routes:check  # только проверка: есть ли маршруты без охраны
```

Вторая команда падает с кодом `1`, если находит эндпоинт, до которого можно дойти без проверки прав, — её удобно вешать в CI или pre-push.
